...
object (self)
  ...
  method sample_of_x x = (int_of_float x) * data_zoomed_block_len + offset

  method private button_press_cb be =
    if GdkEvent.Button.button be = 1 then
      (
        self#set_cursor (self#sample_of_x (GdkEvent.Button.x be));
  ...
  method private button_release_cb be =
    if GdkEvent.Button.button be = 1 then
      (
        let x = self#sample_of_x (GdkEvent.Button.x be) in
          ...
          for c = 0 to self#channels - 1 do
            ...
            for j = 0 to data_zoomed_block_len - 1 do
              ...
              if v < !min then
                ...
                max := v;
              Array2.set data_zoomed_min c i !min;
              Array2.set data_zoomed_max c i !max
            done
          done
        done;
